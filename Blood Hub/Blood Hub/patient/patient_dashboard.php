...
<script>
function initMap() {
    // Initialize the map with a placeholder location (will be updated dynamically)
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(position => {
            ...
            // Fetch and add donor markers
            addDonorMarkers(map);
        }, error => {
            console.error("Error getting user location:", error);
            alert("Unable to retrieve your location. Defaulting to New York.");
            // Fetch and add donor markers even if geolocation fails
            addDonorMarkers(map);
        });
    } else {
        alert("Geolocation is not supported by this browser. Defaulting to New York.");
        // Fetch and add donor markers even if geolocation is not supported
        addDonorMarkers(map);
    }
}
...
function addDonorMarkers(map) {
    $.ajax({
        ...
        type: 'GET',
        success: function(response) {
            console.log("Donors data fetched:", response); // Debugging

            try {
                const donors = JSON.parse(response);
                console.log("Parsed donors data:", donors); // Debugging

                donors.forEach(donor => {
                    console.log("Adding marker for donor:", donor); // Debugging each donor
                    ...
                    new google.maps.Marker({
                        ...
                        map: map,
                        title: donor.name, // Display donor name as marker title
                    });
                });
            } catch (error) {
